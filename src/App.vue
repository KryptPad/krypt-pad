    ...
    <v-app>
        ...
        <v-sheet>
            <title-bar title="Krypt Pad">
                            ...
                            <template v-for="(item, itemIndex) in menu.items" :key="itemIndex">
                                ...
                                >
                                    <div class="d-flex">
                                        ...
                                        <span v-if="item.accelerator" class="ml-auto text-right text-medium-emphasis"
                                            ><span class="ml-3">{{ item.accelerator }}</span></span
                                        >
                                    </div>
                                </v-list-item>
                ...
                <template v-slot:info>
                    {{ windowInfo }}
                    <!-- Timer display -->
                    <template v-if="secondsRemaining"> &nbsp;-&nbsp;{{ formattedTimeRemaining }} </template>
                </template>
            </title-bar>
        </v-sheet>
        ...
        <v-layout>
            ...
            <v-navigation-drawer permanent rail>
                <v-list nav class="d-flex flex-column h-100">
                    ...
                    <v-tooltip text="Home">
                        <template v-slot:activator="{ props }">
                            <v-list-item v-bind="props" prepend-icon="mdi-home-city" title="Home" value="home" :to="homeRoute"></v-list-item>
                        </template>
                    </v-tooltip>

                    <v-tooltip text="Tips">
                        <template v-slot:activator="{ props }">
                            <v-list-item v-bind="props" prepend-icon="mdi-heart" title="Tips" value="tips" :to="{ name: 'tip-me' }"></v-list-item>
                        </template>
                    </v-tooltip>
        ...
        <v-snackbar v-model="timeoutAlert" multi-line :timeout="-1" color="red">
            ...
            <template v-slot:actions>
                <v-btn color="white" @click="resetIdleTimeout"> I NEED MORE TIME </v-btn>
            </template>
        </v-snackbar>
    </v-app>
</template>
